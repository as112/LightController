...
if mqttFlag == 0 then   -- если нет подключения к брокеру
    ...
    m:on("message", function(client, topic, data) 
                if (topic == "/ESP02/R1") and (tonumber(data) == 0) then
                    gpio.write(REL_PIN1,gpio.HIGH)
                    print("R1 ВЫКЛ сверху")
                end
                
                if (topic == "/ESP02/R1") and (tonumber(data) == 1) then
                    gpio.write(REL_PIN1,gpio.LOW)
                    print("R1 ВКЛ сверху")
                end
                
                if (topic == "/ESP02/R2") and (tonumber(data) == 0) then
                    gpio.write(REL_PIN2,gpio.HIGH)
                    print("R2 ВЫКЛ сверху")
                end
                
                if (topic == "/ESP02/R2") and (tonumber(data) == 1) then
                    gpio.write(REL_PIN2,gpio.LOW)
                    print("R2 ВКЛ сверху")
                end

                if (topic == "/ESP02/R3") and (tonumber(data) == 0) then
                    gpio.write(REL_PIN3,gpio.HIGH)
                    print("R3 ВЫКЛ сверху")
                end
                
                if (topic == "/ESP02/R3") and (tonumber(data) == 1) then
                    gpio.write(REL_PIN3,gpio.LOW)
                    print("R3 ВКЛ сверху")
                end
                
                if (topic == "/ESP02/R4") and (tonumber(data) == 0) then
                    gpio.write(REL_PIN4,gpio.HIGH)
                    print("R4 ВЫКЛ сверху")
                end
                
                if (topic == "/ESP02/R4") and (tonumber(data) == 1) then
                    gpio.write(REL_PIN4,gpio.LOW)
                    print("R4 ВКЛ сверху")
                end
                end)
    ...
    m:connect(MQTT_BrokerIP, MQTT_BrokerPort, 0, function(client)
      ...
      client:subscribe({["/ESP02/R1"]=0,
                        ["/ESP02/R2"]=0,
                        ["/ESP02/R3"]=0,
                        ["/ESP02/R4"]=0,}, function(client) print("subscribe success") end)
