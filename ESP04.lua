...
mytimer = tmr.create()
mytimer:register(13000, tmr.ALARM_AUTO, function ()
if mqttFlag == 0 then   -- если нет подключения к брокеру
    ...
    m:on("message", function(client, topic, data) 
                if (topic == "/user/as112/ESP04/R1/") and (tonumber(data) == 0) then
                    gpio.write(REL_PIN1,gpio.HIGH)
					m: publish("/user/as112/ESP04/R1/STATUS/",0,0,0)
                    print("R1 ВЫКЛ сверху")
                end
                
                if (topic == "/user/as112/ESP04/R1/") and (tonumber(data) == 1) then
                    gpio.write(REL_PIN1,gpio.LOW)
					m: publish("/user/as112/ESP04/R1/STATUS/",1,0,0)
                    print("R1 ВКЛ сверху")
                end
                
                if (topic == "/user/as112/ESP04/R2/") and (tonumber(data) == 0) then
                    gpio.write(REL_PIN2,gpio.HIGH)
					m: publish("/user/as112/ESP04/R2/STATUS/",0,0,0)
                    print("R2 ВЫКЛ сверху")
                end
                
                if (topic == "/user/as112/ESP04/R2/") and (tonumber(data) == 1) then
                    gpio.write(REL_PIN2,gpio.LOW)
					m: publish("/user/as112/ESP04/R2/STATUS/",1,0,0)
                    print("R2 ВКЛ сверху")
                end

                if (topic == "/user/as112/ESP04/R3/") and (tonumber(data) == 0) then
                    gpio.write(REL_PIN3,gpio.HIGH)
					m: publish("/user/as112/ESP04/R3/STATUS/",0,0,0)
                    print("R3 ВЫКЛ сверху")
                end
                
                if (topic == "/user/as112/ESP04/R3/") and (tonumber(data) == 1) then
                    gpio.write(REL_PIN3,gpio.LOW)
					m: publish("/user/as112/ESP04/R3/STATUS/",1,0,0)
                    print("R3 ВКЛ сверху")
                end
                
                if (topic == "/user/as112/ESP04/R4/") and (tonumber(data) == 0) then
                    gpio.write(REL_PIN4,gpio.HIGH)
					m: publish("/user/as112/ESP04/R4/STATUS/",0,0,0)
                    print("R4 ВЫКЛ сверху")
                end
                
                if (topic == "/user/as112/ESP04/R4/") and (tonumber(data) == 1) then
                    gpio.write(REL_PIN4,gpio.LOW)
					m: publish("/user/as112/ESP04/R4/STATUS/",1,0,0)
                    print("R4 ВКЛ сверху")
                end
                end)
    ...
    m:connect(MQTT_BrokerIP, MQTT_BrokerPort, 0, function(client)
      ...
      client:subscribe({["/user/as112/ESP04/R1/"]=0,
                        ["/user/as112/ESP04/R2/"]=0,
                        ["/user/as112/ESP04/R3/"]=0,
                        ["/user/as112/ESP04/R4/"]=0,}, function(client) print("subscribe success") end)
      -- publish a message with data = hello, QoS = 0, retain = 0
      -- client:publish("/ESP04/", "hello", 0, 0, function(client) print("ESP04 на связи") end)
    end,
